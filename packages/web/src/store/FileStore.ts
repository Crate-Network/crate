...
> = (set, get): FileState => {
  ...
  const walk = async (path: string): Promise<CID[]> => {
    ...
    const rootCID = segments[1]
    const resolvedCIDs: CID[] = [rootCID].concat(
      await walkDir(rootCID, segments.slice(2))
    )
    return resolvedCIDs
  }
  ...
  const update = async (path: string, updatedFile: Partial<FileModel>) => {
    ...
    while (allCIDs.length > 0) {
      ...
      newModel.cid = nextCID
      console.log(`created ${nextCID}`)
      set(({ files }) => {
        files[newModel.cid] = newModel
      })
    }

    const newPath = `/ipfs/${nextCID}/${splitPath(path).slice(2).join("/")}`
    return newPath
  }
